...
class ToneGeneratorBot2:
    ...
    def execute(self):
        ...
        a = int(round(float(input())))
        # set the frequency
        #self._driver.find_element(By.ID, 'freq-readout').click()
        freq_text = self._driver.find_element(By.ID, 'freq-readout')
        #element = self._driver.switch_to.active_element
        #element.send_keys(Keys.CONTROL + 'a' + f)
        #iinput = self._driver.find_element(By.CSS_SELECTOR, '#freq-readout input')
        actions = ActionChains(self._driver)
        actions.move_to_element(freq_text).click().send_keys(Keys.CONTROL, 'a', f).perform()
        #actions.send_keys_to_element(Keys.CONTROL + 'a').send_keys_to_element(f).perform()
        #print(iinput.text)
        #iinput.text.replace("440", f)
        #iinput.send_keys(Keys.CONTROL, 'a', Keys.BACKSPACE, f)
        # set the amplitude
        self._driver.find_element(By.ID, 'play-button').click()
